...
function generateMediaQA(userInfos,mediaText, avatarIcon, modal){
...
function appendAnwser(mediaQuery, mediaAnswer){
  ...
  mediaQuery.getElementsByClassName("hero hero-sm mx-2 answer")[0].appendChild(answer)
}
...
function generateAllMediaQueries(array, observer){
  ...
  for(var i  = 0; i< array.length; i++){
    const obj = array[i];
    const mediaObject = generateMediaQA(obj.userInfos, obj.mediaText, obj.avatarIcon, obj.modal)
...
function modalButtonGenerator(func, text="Click to open the media"){
  ...
  button.classList.add("btn","s-rounded")
  button.title = text
  button.onclick = func
  return button
}
